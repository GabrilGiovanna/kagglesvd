...
def evaluate(rating, hyper_params, data, item_propensity, train_x, topk = [1, 5, 10, 20, 50, 100 ], test_set_eval = False):
    ...
    groups = GROUPING_STRATEGIES[hyper_params['grouping_method']].load_with_best_hyperparams(train,hyper_params['aggregation'], 'SVD-AE')
    ...
    print('Time taken to aggregate: {}'.format(time.time()-start))
    ...
    print('Time taken to evaluate: {}'.format(time.time()-start))
    preds += temp_preds
    y_binary += temp_y
